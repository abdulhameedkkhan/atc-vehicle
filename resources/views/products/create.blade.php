        ...
        <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <select name="cnf_fob_type" id="cnf_fob_type"
                        ...
                        <option value="CNF" {{ old('cnf_fob_type') == 'CNF' ? 'selected' : '' }}>CNF</option>
                        <option value="FOB" {{ old('cnf_fob_type') == 'FOB' ? 'selected' : '' }}>FOB</option>
                    </select>
                </div>
                <div>
                    <label for="cnf_fob_price" class="block text-sm font-medium text-gray-700 mb-2">CNF/FOB Price: Amount</label>
                    <input type="number" step="0.01" name="cnf_fob_price" id="cnf_fob_price" value="{{ old('cnf_fob_price') }}"
            ...
            <div class="mb-6">
                ...
                <select name="status" id="status"
                    ...
                    <option value="stock" {{ old('status', 'stock') == 'stock' ? 'selected' : '' }}>Stock</option>
                    <option value="reserved" {{ old('status') == 'reserved' ? 'selected' : '' }}>Reserved</option>
                    <option value="sold" {{ old('status') == 'sold' ? 'selected' : '' }}>Sold</option>
                    <option value="ship" {{ old('status') == 'ship' ? 'selected' : '' }}>Ship</option>
                </select>
            </div>
